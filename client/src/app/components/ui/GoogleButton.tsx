...
export default function GoogleButton() {
  ...
  const login = useGoogleLogin({
    flow: "implicit",          // returns an access_token
    onSuccess: async (token) => {      
      try {
        // Send Google access_token -> backend
  ...
  /* -- Styled button (outline -> fill on hover) -- */
  return (
    <button
      type="button"
      onClick={() => login()}
      className="group relative flex items-center justify-center w-full border border-gray-300 px-5 py-1.5 text-sm tracking-wide overflow-hidden"
    >
      ...
      {/* slide-in fill */}
      <span className="absolute inset-0 bg-gray-900 translate-y-full group-hover:translate-y-0 transition-transform duration-300" />
      <style jsx>{`
        button {
          color: #000;
        }
